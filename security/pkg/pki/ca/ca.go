...
type IstioCA struct {
	// 默认的证书有效期
	defaultCertTTL time.Duration
	// 最大有效期，用于限制Citadel可签发证书的最大有效期，当请求签发的证书TTL超过最大有效期时，IstioCA将会返回签发失败错误。
	maxCertTTL   time.Duration
	caRSAKeySize int

	// 具有本地缓存功能的抽象接口，缓存CA颁发证书所用的材料（公私钥对、根证书等）。
	keyCertBundle *util.KeyCertBundle
