...
func newDiscoveryCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			// 创建xDS服务器，即Pilot Server对象。
			// Pilot Server其实就是注册中心与Envoy代理之间的桥梁，它将服务及配置资源转化成xDS配置，再通过gRPC连接将xDS配置发送给Envoy代理。
			...
			// Start the server
			// 启动服务器。Pilot Server的启动通过执行其所有模块的启动函数startFuncs 完成
			// 在模块初始化时都会通过func （s *Server） addStartFunc（fn startFunc）接口注册自己的启动函数到Server对象的startFuncs属性中。
